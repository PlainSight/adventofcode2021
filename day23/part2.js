...
var grid = fs.readFileSync('./input2.txt', 'utf8').split('\r\n').map(l => l.split(''));
        ...
        if (!piece.fin) {
            ...
            if (piece.moved) {
                ...
                if (destBlockers.length == 0 && hallBlockers.length == 0) {
                    // put in destination
                    var y = 5 - pieces.filter(p => p.x == piece.destCol && p.fin).length;
                    moves.push({ id: i, x: piece.destCol, y: y, cost: ((y-1)+Math.abs(piece.destCol-piece.x))*req[piece.letter] });
                }
            } else {
