...
scannerOffsets.forEach(so1 => {
    scannerOffsets.forEach(so2 => {
        if (so1 != so2) {
            var dist = Math.abs(so1[0] - so2[0]) + Math.abs(so1[1] - so2[1]) + Math.abs(so1[2] - so2[2]);
            if (dist > largestDist) {
                largestDist = dist;
            }
        }
